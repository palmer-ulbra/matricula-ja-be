...
export const dentroDoPrazo = (p: PeriodoLetivo, agora: Date) => {
  return agora >= p.inicio_matricula && agora <= p.fim_matricula;
};

export const erroForaDoPrazo = (p: PeriodoLetivo) => {
  return new ErroDaApi(422, 'FORA_DO_PRAZO', 'RN-4', `Fora do prazo de matrícula do período ${p.codigo}`);
};

// ── RN-7 · Duplicidade ──────────────────────────────────────────────────────
export const estadoOcupaTurma = (e: EstadoMatricula) => {
  return e === 'PENDENTE' || e === 'CONFIRMADA';
};

export const erroDuplicidade = () => {
  return new ErroDaApi(409, 'MATRICULA_DUPLICADA', 'RN-7', 'Matrícula já existente para esta turma');
};

// ── RN-2 · Pré-requisito ────────────────────────────────────────────────────
export const erroPreRequisito = (codigo: string, nome: string) => {
  return new ErroDaApi(422, 'PRE_REQUISITO_NAO_CUMPRIDO', 'RN-2', `Pré-requisito não cumprido: ${codigo} ${nome}`);
};
...
export const horariosChocam = (
  ...
  return a.dia_semana === b.dia_semana && a.hora_inicio < b.hora_fim && b.hora_inicio < a.hora_fim;
};
...
export const creditosDoSemestre = (matriculas: { estado: EstadoMatricula; creditos: number }[]) => {
  return matriculas.reduce((total, m) => {
    return total + m.creditos;
  }, 0);
};

export const alunoIrregular = (creditos: number) => {
...
export const temVaga = (t: Pick<Turma, 'vagas_totais' | 'vagas_ocupadas'>) => {
  return t.vagas_ocupadas < t.vagas_totais;
};

export const erroSemVaga = () => {
  return new ErroDaApi(409, 'TURMA_SEM_VAGA', 'RN-1', 'Turma sem vaga disponível');
};
